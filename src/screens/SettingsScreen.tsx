...
const Toggle: React.FC<{ on: boolean; onToggle: () => void }> = ({
  ...
  <button
    ...
    <div
      className={`absolute top-1 w-3 h-3 rounded-full transition-all duration-300 ${on ? "bg-black right-1 shadow-sm" : "bg-white/40 left-1"}`}
    />
  </button>
)
          ...
          <>
            <div className="bg-red-500/10 border border-red-500/20 rounded-xl p-3 mb-4 flex gap-2">
              ...
              <p className="text-red-400/80 text-xs leading-relaxed">
                Never share this with anyone. Anyone with this can steal your
                funds permanently.
              </p>
            </div>
            <p className="text-white/40 text-xs mb-4 leading-relaxed">
              Enter your wallet password to decrypt and view your secret.
            </p>
            <input
              type="password"
              value={password}
            ...
            <div className="flex gap-3 mt-3">
              ...
              <button
                ...
                className="flex-1 py-3 bg-red-500/80 text-white rounded-xl text-xs font-black disabled:opacity-30 hover:bg-red-500 active:scale-[0.98] transition-all">
          ...
          <>
            ...
            <div className="relative bg-black/60 border border-white/10 rounded-xl p-4 mb-4">
              <button
                ...
                className="absolute top-3 right-3 w-7 h-7 rounded-lg bg-white/10 flex items-center justify-center hover:bg-white/20 transition-all">
                {showSecret ? (
              ...
              <p
                className={`text-sm font-mono break-all leading-relaxed pr-8 transition-all ${showSecret ? "text-white select-all" : "blur-sm select-none text-white/50"}`}>
                {secret}
              </p>
              {!showSecret && (
                <button
                  onClick={() => setShowSecret(true)}
                  className="absolute inset-0 flex items-center justify-center text-white/40 text-xs font-bold">
                  Click to reveal
                </button>
              )}
            </div>
            <div className="flex gap-3">
              <button
                onClick={handleCopy}
                className={`flex-1 py-3 rounded-xl text-xs font-black transition-all ${copied ? "bg-emerald-400 text-black" : "bg-white/10 text-white hover:bg-white/15"}`}>
                {copied ? "COPIED!" : "COPY"}
              </button>
              <button
                onClick={onClose}
...
// Network options — EVM chains with logos from SUPPORTED_CHAINS
const NETWORK_OPTIONS = SUPPORTED_CHAINS.filter(
...
export const SettingsScreen: React.FC<Props> = ({
  ...
  const [modal, setModal] = useState<"privateKey" | "seedPhrase" | null>(null)

  useEffect(() => {
    chrome.storage.local.get(
      ["zeno_address", "zeno_lock_limit", "zeno_chain_filter"],
      (res) => {
        ...
        if (res.zeno_chain_filter && res.zeno_chain_filter !== "all")
          setNetwork(res.zeno_chain_filter)
      }
    )
  }, [])
  ...
  const handleRevealPrivateKey = async (password: string) => {
    try {
      ...
      if (!res.zeno_vault || !res.zeno_salt)
        return { secret: "", error: "Vault not found." }
      ...
      if (!mnemonic || mnemonic.split(" ").length < 12)
        return { secret: "", error: "Incorrect password." }
  ...
  const handleNetworkChange = (v: string) => {
    setNetwork(v)
    // Sync with dashboard ChainSelector
  ...
  const handleReset = async () => {
    if (
      confirm("Reset wallet? This will delete all data and cannot be undone!")
    ) {
    ...
    <div className="flex-1 flex flex-col overflow-y-auto p-4 custom-scrollbar animate-fade-up relative">
      {modal === "privateKey" && (
        <SecretModal
          ...
          onReveal={handleRevealPrivateKey}
        />
      )}
      {modal === "seedPhrase" && (
        <SecretModal
